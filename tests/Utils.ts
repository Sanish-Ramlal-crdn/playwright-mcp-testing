...
export async function addProductToCart(request: APIRequestContext, cartId: string, productId: string, quantity = 1) {
...
export async function getProductById(request: APIRequestContext, productId: string) {
  const getProductRes = await request.get(`https://api.practicesoftwaretesting.com/products/${productId}`);
...
export async function getValidToken(request: APIRequestContext, userFixturePath = "tests/fixtures/apiUser.json") {
  let tokenData;
  const tokenPath = path.resolve(process.cwd(), userFixturePath.replace('apiUser.json', 'token.json'));
  try {
...
export async function loginOrRegisterUI(authPage: any, user: any, urls: any, page: Page) {
  await authPage.goto();
